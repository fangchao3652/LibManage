(function($) {
    $.fn.jquizzy = function(settings) {
        ...
        exitFob = '<div class="results-container slide-container"><div class="question-number"><a href="'+config.BaseUrl+'/pre_study.jsp">填写预习报告</a></div><div class="result-keeper"></div></div><div class="notice">请选择一个选项！</div><div class="progress-keeper" ><div class="progress"></div></div>',
        contentFob = '',
        questionsIteratorIndex,
        answersIteratorIndex;
        superContainer.addClass('main-quiz-holder');
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
            contentFob += '<div class="slide-container"><div class="question-number">' + (questionsIteratorIndex + 1) + '/' + config.questions.length + '</div><div class="question">' + config.questions[questionsIteratorIndex].topic + '</div><ul class="answers">';
            for (answersIteratorIndex = 0; answersIteratorIndex < eval(config.questions[questionsIteratorIndex].options).length; answersIteratorIndex++) {
                contentFob += '<li>' + eval(config.questions[questionsIteratorIndex].options)[answersIteratorIndex] + '</li>';
            }
            ...
            contentFob += '</div></div>';
            answers.push(config.questions[questionsIteratorIndex].answer);
        }
        ...
        superContainer.find('.final').click(function() {
            ...
            toLoopTill = results.length; i < toLoopTill; i++) {
                ...
                resultSet += '<div class="resultsview-qhover">' + config.questions[i].topic;
                resultSet += "<ul>";
                for (answersIteratorIndex = 0; answersIteratorIndex <eval(config.questions[i].options).length; answersIteratorIndex++) {
                    var classestoAdd = '';
                    if (config.questions[i].answer == answersIteratorIndex + 1) {
                        classestoAdd += 'right';
                    }
                    if (userAnswers[i] == answersIteratorIndex + 1) {
                        classestoAdd += ' selected';
                    }
                    resultSet += '<li class="' + classestoAdd + '">' + eval(config.questions[i].options)[answersIteratorIndex] + '</li>';
                }
